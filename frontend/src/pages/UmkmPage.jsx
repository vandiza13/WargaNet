...
import { Plus, Search, MapPin, Phone, CheckCircle, Clock } from 'lucide-react';
    ...
    return (
        <div className="animate-fade-in-up">
            <div className="text-center mb-8">
                <h1 className="text-3xl font-bold text-gray-800">Direktori UMKM Warga</h1>
                <p className="text-gray-500 mt-2">Dukung dan temukan produk serta jasa dari tetangga Anda.</p>
                <button onClick={() => setIsModalOpen(true)} className="mt-4 flex items-center mx-auto bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700 transition-colors">
                    <Plus size={16} className="mr-2" /> Daftarkan UMKM Saya
                </button>
            </div>

            {myUmkmList.length > 0 && (
                <div className="mb-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h2 className="text-lg font-bold text-gray-800 mb-3">UMKM Saya</h2>
                    <div className="space-y-2">
                        {myUmkmList.map(myUmkm => (
                            <div key={myUmkm._id} className="bg-white p-3 rounded-md flex justify-between items-center shadow-sm">
                                <p className="font-semibold">{myUmkm.name}</p>
                                <div className={`p-2 rounded-full flex items-center text-xs ${myUmkm.isApproved ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                    {myUmkm.isApproved ? <CheckCircle size={14} className="mr-1.5" /> : <Clock size={14} className="mr-1.5" />}
                                    {myUmkm.isApproved ? 'Disetujui' : 'Menunggu'}
                                </div>
                            </div>
                        ))}
                    </div>
